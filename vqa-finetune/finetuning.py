...
exp_name = f"{config['model']['name']}_{config['model']['ft']}_{config['model']['type']}"
...
if config["data"]["format"] == "csv":
    from data import VQACSVDataset
    ...
    train_dataset = VQACSVDataset(train_df, processor, is_train=True)
    val_dataset = VQACSVDataset(val_df, processor, is_train=False)
    ...
    train_dataset = VQADataset(dataset=training_dataset,
                            processor=processor)
    val_dataset = VQADataset(dataset=valid_dataset,
                            processor=processor)
